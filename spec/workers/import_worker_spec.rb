...
describe ImportWorker do
  ...
  context 'upon initialization' do
    it 'starts the import process from AngelList API' do
      ImportWorker.any_instance.should_receive(:run).once
      ImportWorker.new.perform(user.id, startup_id)
    end
  end

  context 'instance' do
    let(:import) { ImportWorker.new }

    context '#run' do
      before do
        # TODO: Find a way to avoid any_instance stub
        ImportWorker.any_instance.stub(:import_project).and_return(false)
        UserMailer.should_receive(:startup_imported).exactly(0).times
        import.perform(user.id, startup_id)
      end
    ...
    context '#run on success sends an email and flags importing as false' do
      let(:project) { Fabricate(:project, :user => user) }
      before do
        # TODO: Find a way to avoid any_instance stub
        ImportWorker.any_instance.stub(:import_project).and_return(project)
        ImportWorker.any_instance.stub(:import_project_comments)
    ...
    context '#import_project' do
      ...
      before do
        # TODO: Find a way to avoid any_instance stub
        ImportWorker.any_instance.stub(:process_json).and_return(startup)
        import.user = user
    ...
    context '#import_project_comments' do
      ...
      before do
        # TODO: Find a way to avoid any_instance stub
        ImportWorker.any_instance.stub(:process_json).and_return(data)
        import.project = project
